...
// Read current/old peppers from env
const CURRENT_PEPPER = process.env.PEPPER || '';
const OLD_PEPPERS = (process.env.PEPPER_OLD || '').split(',').map(s => s.trim()).filter(Boolean);
...
async function verifyPasswordWithPeppers(storedHash, prehashedPassword) {
  for (const pepper of PEPPERS) {
    try {
      if (await argon2.verify(storedHash, pepper + prehashedPassword)) return pepper;
    } catch (e) {
      // ignore
    }
  }
